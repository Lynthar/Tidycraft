...
- **Issue titles and explanations now follow the interface language.** The analysis panel's rule names and descriptions were English regardless of the language setting, sitting next to Chinese labels. Rules translate one at a time — anything not yet translated keeps its English text rather than showing a placeholder — and the exported HTML report picks up the same titles and explanations, though its own headings, labels and column names are still English. The JSON export stays English throughout, as a stable contract for scripts. It does pick up one addition: every issue that interpolates a value into its message now carries those values in a machine-readable `args` object (21 of the 23 rules), for anyone parsing that export who wants to render the text themselves. Audio sample-rate issues also stop printing the expected rates in Rust's debug form (`[44100, 48000]`).
