...
No configuration. Files are grouped by size first (cheap), then by a hash of their opening 8 KB, and only files still sharing both are SHA256-hashed in full to confirm true content equality. The middle pass is what keeps a texture library cheap: block-compressed images of the same dimensions and format have identical byte counts, so size alone leaves them all in one group. Each content group produces **one** `duplicate` warning that lists every member via `related_paths` (root-relative, lexicographically-first "original" leading); the issue itself anchors on the first redundant copy so Locate lands on a file you can act on. The Issues view renders these as a single group card.
