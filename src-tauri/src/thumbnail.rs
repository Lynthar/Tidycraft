...
/// Cache key from a file's path, mtime (nanoseconds) and size. Nanoseconds
/// separate a source image from a same-second replacement; size covers the
/// filesystems that store whole seconds only.
...
fn save_to_cache(cache_key: &str, data: &[u8]) -> Result<(), ThumbnailError> {
    if let Some(cache_dir) = get_cache_dir() {
        ...
        let cache_path = cache_dir.join(format!("{}.png", cache_key));
        // Atomic (unique temp + rename): two concurrent requests for the same
        // key would otherwise interleave and leave a torn PNG cached.
...
pub fn get_thumbnail_base64(path: &str, max_size: u32) -> Result<String, ThumbnailError> {
    ...
    // Cargo.toml. HDR/EXR lose dynamic range when written out as 8-bit PNG.
...
fn generate_thumbnail(path: &Path, max_size: u32) -> Result<Vec<u8>, ThumbnailError> {
    ...
    // PNG has no 32-bit float channels, so HDR/EXR (decoded as Rgb32F / Rgba32F)
    // must be flattened to 8-bit first. `to_rgba8` clamps values > 1.0;
    // non-float images pass through untouched.
...
/// Ceiling for the thumbnail cache, swept once per launch. A 256px preview runs
/// 30–80 KB, so this holds several thousand of them.
const CACHE_MAX_BYTES: u64 = 256 * 1024 * 1024;

/// Delete cache files, oldest first, until the directory fits in `max_bytes`.
/// Returns the bytes reclaimed. Age is when the thumbnail was *generated*, not
/// when it was last shown — true LRU would mean a write on every cache hit.
fn prune_dir(dir: &Path, max_bytes: u64) -> std::io::Result<u64> {
    ...
    for (_, len, path) in files {
        ...
        // A file that won't delete must not stop the sweep.
...
mod tests {
    ...
    /// Write a `bytes`-sized file at `name` with a pinned mtime.
    ...
    fn cache_key_separates_rewrites_inside_one_second() {
        // A key that rounds the mtime down to the second cannot tell a
        // re-export from the file it replaced.
        let dir = tempfile::tempdir().unwrap();
    ...
    fn generate_thumbnail_flattens_hdr_float_to_png() {
        // `image::open` decodes .hdr to Rgb32F, which `PngEncoder` rejects, so
        // float pixels must be flattened to 8-bit first. A value > 1.0 exercises
        // real HDR data.
        let dir = tempfile::tempdir().unwrap();
